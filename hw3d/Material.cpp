	...
	modelPath( path.string() )
{
	...
	// phong technique
	{
		...
		// common (post)
		{
			...
			buf["specularWeight"].SetIfExists( 0.0f );
...
Material::Material(bool solid, Graphics& gfx, const aiMaterial& material, const std::filesystem::path& path) noxnd
	:
modelPath(path.string())
{
	using namespace Bind;
	...
	// phong technique
	{
		...
		// common (post)
		{
			...
			auto pvsbc = pvs->GetBytecode();
			step.AddBindable(std::move(pvs));
			step.AddBindable(PixelShader::Resolve(gfx, "LightedTexture_PS.cso"));
			step.AddBindable(InputLayout::Resolve(gfx, vtxLayout, pvsbc));
			if (hasTexture)
			{
				step.AddBindable(Bind::Sampler::Resolve(gfx));
